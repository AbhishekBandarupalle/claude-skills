# Encountered Issues — Sylva OKD Management Cluster (cabpoa/capm3)
...
## Phase: CoreDNS on OKD

### CoreDNS enabled_conditions exclude cabpoa

**Symptom**: `coredns` and `coredns-config` kustomizations don't get created on OKD.  
**Root cause**: `coredns` has `enabled_conditions` for `cabpk`/`cabpr` only; `coredns-config` has `ne "cabpoa"`.  
**Fix**: Add `cabpoa` to `coredns` enabled_conditions, remove the `ne "cabpoa"` condition from `coredns-config`.  
**File**: `charts/sylva-units/values.yaml`  
**Note**: rke2-coredns on OKD runs as a secondary DNS — nothing queries it by default. For in-cluster Ingress resolution, configure OKD DNS operator `forwardPlugin` instead.

### CoreDNS-config health check fails (chicken-and-egg)

**Symptom**: `coredns-config` kustomization stuck with `HealthCheckFailed: timeout waiting for Deployment/kube-system/rke2-coredns-rke2-coredns status: 'NotFound'`.  
**Root cause**: `coredns-config` has a health check waiting for the rke2-coredns Deployment, but that deployment is created by the `coredns` HelmRelease which depends on `coredns-config`. On RKE2, CoreDNS already exists; on OKD it doesn't.  
**Fix**: Remove the `healthChecks` from `coredns-config` in `charts/sylva-units/values.yaml`. The `coredns` kustomization checks HelmRelease health instead.

### CoreDNS HelmRelease ClusterIP conflict

**Symptom**: `coredns` HelmRelease fails with `Service "rke2-coredns-rke2-coredns" is invalid: spec.clusterIPs: provided IP is already allocated`.  
**Root cause**: The rke2-coredns chart defaults `global.clusterDNS` to `.10` of the services CIDR (e.g. `100.73.0.10`), which is already used by OKD's `dns-default` service.  
**Fix**: Override in environment values to let Kubernetes auto-assign:
...
**File**: `environment-values/my-okd-capm3/values.yaml`

### rke2-coredns running but nothing queries it

**Symptom**: rke2-coredns pods are Running but Ingress hostnames don't resolve from inside pods.  
**Root cause**: OKD kubelet points pods to `openshift-dns` (100.73.0.10), not rke2-coredns. The k8s-gateway forwarding in rke2-coredns Corefile is unreachable.  
**Fix**: This is expected. To get in-cluster Ingress resolution, configure OKD DNS operator instead:
...
**Symptom**: `oc` commands fail with `tls: failed to verify certificate: x509: certificate has expired or is not yet valid`.  
**Root cause**: Bastion clock is behind the OKD node. New kubeconfig certs issued by the cluster have a "not before" timestamp in the bastion's future.  
**Fix**: Sync the bastion clock from the OKD node:

```bash
ssh-keygen -f ~/.ssh/known_hosts -R <NODE_IP>
NODE_TIME=$(ssh -i ~/.ssh/ocp_ssh_key -o StrictHostKeyChecking=no core@<NODE_IP> "date -u '+%Y-%m-%d %H:%M:%S'")
sudo date -u -s "$NODE_TIME"
```

### SSH host key changed after redeploy

**Symptom**: SSH fails with `REMOTE HOST IDENTIFICATION HAS CHANGED`.  
**Root cause**: Node was redeployed with a new host key.  
**Fix**: `ssh-keygen -f ~/.ssh/known_hosts -R <NODE_IP>`
